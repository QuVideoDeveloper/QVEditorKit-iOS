...
{
    self.defaultTemplateVersion = defaultTemplateVersion;
    [self scanDB];
...
{
    BOOL needUpdate = NO;
    int currentTemplateVesion = self.defaultTemplateVersion;
...
- (XYTemplateItemData*)getByPath:(NSString*)path {
    ...
    dispatch_sync(_queue, ^{
        for(XYTemplateItemData* item in mTemplateArray) {
            if([path isEqualToString:item.strPath]){
                param = item;
                break;
            }
        }
...
- (NSString *)getTemplateExternalFile:(MInt64)templateID SubTemID:(MDWord)subTemplateID FileID:(MDWord)fileID
{
    ...
    if([filePath hasSuffix:APP_TEMPLATE_EXT] ){
        ...
        if(![self getByPath:filePath]){//Patch for missing db data for the internal folder xyt files
...
- (NSString *)getTitle:(XYTemplateItemData *)XYTemplateItemData {
    NSString *current = [[XYStoryboard sharedXYStoryboard] fetchLanguageCode];
...
{
     [self install:strTemplateFile isDeleteOriginFile:YES];
}

- (void)install:(NSString*)strTemplateFile isDeleteOriginFile:(BOOL)isDeleteOriginFile
{
     [self install:strTemplateFile fromType:FROM_TYPE_DOWNLOAD force:YES isDeleteOriginFile:isDeleteOriginFile];
}
