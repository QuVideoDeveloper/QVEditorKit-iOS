...
typedef NS_ENUM(NSInteger, XYCommonEnginebackgroundType) {
    XYCommonEngineBackgroundNormal = 0,
...
@property (nonatomic, assign) NSInteger backgroundColorStartValue;//背景颜色 渐变起始颜色
@property (nonatomic, assign) NSInteger backgroundColorEndValue;//背景颜色 渐变结束颜色 格式（0Xffffff）
...
//移动 需要的参数 1.shiftX 2.shiftY（没做移动默认值都是1,shiftX 是移动的X除以播放器的的宽(playbackView.width),shiftY同理）
