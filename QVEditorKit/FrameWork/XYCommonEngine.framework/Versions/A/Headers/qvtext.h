...
//	MDWord dwGStartIdx;//本行第一个Glyph在String中的索引值
	MDWord dwGCnt;	//本行所包含的Glyph count
}QVTE_LINE_INFO;

typedef struct __tagQVTE_PARAGRAPH_MEASURE_RESULT
{
//	MSIZE  BGSize; 	//result size of measuring to the input QVTE_PARAGRAPH_STYLE，BGSize 可能是2的N次方，与实际图像需要的大小是有出入的，主要考虑openGL上传的性能问题问题
	...
	MByte  *pGFlagList; // 标识纹理区域状态类型的数组 每个元素值是0则正常字符，是val&1>0有阴影特性，是val&2>0 则有模糊特性
	MDWord dwGCnt;	//G =glyph

	QVTE_LINE_INFO *pLineInfoList;
	MDWord dwLines;

//	MTChar *pszResult;	//被处理过字符串，已经按要求插入换行符号
